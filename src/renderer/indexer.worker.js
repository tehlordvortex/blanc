...
async function index (path) {
  ...
  // let resolves = []
  let files = await promiseFiles(path)
  files = files.filter(file => mime.getType(file) && mime.getType(file).startsWith('audio'))
  let items = []
  for (const file of files) {
    ...
    try {
      ...
      console.log('Error indexing:', file, e)
      continue
    }
    if (metadata) {
      ...
      console.log('No metadata for file', file)
    }
  }
